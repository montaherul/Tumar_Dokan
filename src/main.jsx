...
const router = createBrowserRouter([
  ...
  },
  {
    ...
    loader: async ({ request }) => { // Access request object to get URL search params
      ...
      const response = await fetch(`https://tumar-dokan-v0.onrender.com/api/products?${query.toString()}`);
      if (!response.ok) {
  ...
  {
    path: "/productfromapi",
    loader: () => fetch("https://tumar-dokan-v0.onrender.com/api/products"), // Keep this for now if it's a separate feature
    Component: Productfromapi,
  },
  {
    path: "/products/:id",
    loader: ({ params }) => {
      return fetch(`https://tumar-dokan-v0.onrender.com/api/products/${params.id}`); // Use new backend API
    },
    Component: Details,
  },
